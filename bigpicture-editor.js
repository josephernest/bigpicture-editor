var bigpicture = (function() {
  ...
  document.getElementById("close").addEventListener("click", function (e) { remote.getCurrentWindow().close(); }); 

  var bpContainer = document.getElementById('bigpicture-container');
  var searchInput = document.getElementById('search');
  const fs = require('fs');
  const path = require('path');
  var datafile = path.join(__dirname, 'bigpicture.data');
  bpContainer.innerHTML = fs.readFileSync(datafile);

  window.onunload = function() { 
    bp.style = '';
    bp.dataset.x = current.x;
    bp.dataset.y = current.y;
    bp.dataset.zoom = current.zoom;
    $(".text").each(function() {
     this.style = ''; this.dataset.x = $(this).data('x'); this.dataset.y = $(this).data('y'); this.dataset.size = $(this).data('size'); 
    });
    fs.writeFileSync(datafile, bpContainer.innerHTML, "UTF-8", {'flags': 'w+'});
  }
  ...
  window.onkeydown = function(e) {
    ...
    if (e.keyCode == 114) {                 // F3
      e.preventDefault();
      if (results.index == -1) { $('#search').show().focus(); } else { findNext(query); }
      return;
    }
    if (e.keyCode == 113) {                 // F2
      e.preventDefault();
  ...
  searchInput.onkeydown = function(e) { 
    if (e.keyCode == 13) { query = $('#search').val(); findNext(query); }    // enter
    if (e.keyCode == 27) { $('#search').hide(); }                            // escape
  };
  searchInput.onblur = function(e) { $('#search').hide(); };
